...
<div class="row">
    ...
    <div id="import_keyword" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header bg-info white">
                        <h4 class="modal-title white" id="myModalLabel11"><i class="la la-tree"></i>{{ trans('Import Keyword') }}</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close" data-dismiss-modal="#import_keyword">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    {!! Form::open(['enctype' => 'multipart/form-data','id' => 'form-import-keyword']) !!}
                    <div class="modal-body with-padding">
                        <div class="form-group">
                            <label>{{ trans('File csv') }}</label>
                            {!! Form::file('csv', null, ['class' => 'form-control']) !!}
...
@section('master-footer')
    <script type="text/javascript">
        ...
        $('#import_keyword').on('shown.bs.modal', function (e) {
            $('#form-import-keyword')[0].reset();
        })
    </script>
    <script type="text/javascript">
        $(document).on('click', '.importKeywordDialog', function (event) {
            event.preventDefault();
            $('#form-import-keyword').attr('action', $(this).data('keyword-url'));
            $('#import_keyword').modal('show');
        });
    </script>
@endsection