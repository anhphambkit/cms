...
Lcms.initMediaIntegrate = function () {
    if (jQuery().rvMedia) {
        ...
        $(document).on("click", '.btn_remove_look_book_image', function(event) {
            event.preventDefault();
        ...
        $('.btn_select_gallery').rvMedia({
            onSelectFiles: function (files, $el) {
                let galleryBox = $el.parents('.gallery-image-box');
                var result = galleryBox.find('.gallery-data');
                var images = [];
                var last_index = 0;
                if (result.val() !== '') {
                    images = $.parseJSON(result.val());
                    last_index = galleryBox.find('.list-photos-gallery .photo-gallery-item:last-child').data('id') + 1;
                }
                $.each(files, function (index, file) {
                    images.push(file.url);
                    galleryBox.find('.list-photos-gallery .row').append('<div class="col-md-2 col-sm-3 col-4 photo-gallery-item" data-id="' + (last_index + index) + '"><div class="gallery_image_wrapper"><img src="' + file.thumb + '" /></div></div>');
                });
                result.val(JSON.stringify(images));
                galleryBox.find('.reset-gallery').removeClass('hidden');
            }
        });
        ...
        $(document).on("click", '.reset-gallery', function(event) {
            event.preventDefault();
            let galleryBox = $(this).parents('.gallery-image-box');
            galleryBox.find('.list-photos-gallery .photo-gallery-item').remove();
            galleryBox.find('.gallery-data').val('');
        ...
        list_photo_gallery.on('click', '.photo-gallery-item', function () {
            var id = $(this).data('id');
            let galleryBox = $(this).parents('.gallery-image-box');
            galleryBox.find('.delete-gallery-item').data('id', id);
            galleryBox.find('.update-gallery-item').data('id', id);
            var images = $.parseJSON(galleryBox.find('.gallery-data').val());
            if (images != null && typeof images[id] != 'undefined') {
                galleryBox.find('.edit-gallery-item .gallery-item-description').val(images[id].description);
            }
            galleryBox.find('.edit-gallery-item').modal('show');
        });

        edit_gallery_modal.on('click', '.delete-gallery-item', function (event) {
            event.preventDefault();
            let galleryBox = $(this).parents('.gallery-image-box');
            galleryBox.find('.edit-gallery-item').modal('hide');
            var id = $(this).data('id');
            var parent = galleryBox.find('.list-photos-gallery .photo-gallery-item[data-id=' + $(this).data('id') + ']');
            var images = $.parseJSON(galleryBox.find('.gallery-data').val());
            var newListImages = [];
            ...
            galleryBox.find('.gallery-data').val(JSON.stringify(newListImages));
            parent.remove();
            if (galleryBox.find('.list-photos-gallery .photo-gallery-item').length === 0) {
                galleryBox.find('.reset-gallery').addClass('hidden');
            }
        });
        ...
        edit_gallery_modal.on('click', '.update-gallery-item', function (event) {
            ...
            if (images != null && typeof images[id] != 'undefined') {
                images[id].description = galleryBox.find('.edit-gallery-item .gallery-item-description').val();
            }
            result.val(JSON.stringify(images));
        });
    }
